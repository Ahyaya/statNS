...
int main(){
    ...
    Path_t EoSlist[1000];/*用来存放EoS的文件路径，这里能存1000个*/
...
/*初始化一个数组来存放计算结果，它未必要跟EoS的总数量一样长，因为你可以分多次进行计算*/
    CompactStar_t Results[EoSlistLen];

/*这里指定用6个线程进行计算，结果存放到Results[]上，目标质量为1.654321倍太阳质量，EoS文件名列表入口，总共要计算的EoS数量*/
    m2fmode_mt(Results,1.654321,EoSlist,EoSlistLen,6);
